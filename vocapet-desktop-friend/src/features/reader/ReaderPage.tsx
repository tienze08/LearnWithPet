...
import type { BasketItem, VocabularySuggestion } from "./types";
import { sentenceAround, speakWord } from "./utils/text";
...
function ReadingParagraph({
  ...
  return <>{text.split(/(\b[\p{L}][\p{L}'-]*\b)/u).map((token, index) => {
    ...
    return (
      <span key={`${token}-${index}`} className="group relative inline-block mx-0.5">
        <span className={`cursor-help rounded px-0.5 font-semibold underline decoration-2 underline-offset-4 ${inBasket ? "bg-amber-100 decoration-amber-400" : "bg-primary/15 decoration-primary"}`}>
          {token}{inBasket && <sup className="ml-0.5 text-[9px] no-underline">📚</sup>}
        </span>
        <span className="pointer-events-none absolute bottom-full left-1/2 z-30 mb-2 hidden w-60 -translate-x-1/2 rounded-2xl border-2 border-border bg-card p-3 text-left text-xs shadow-xl group-hover:block group-focus-within:block">
          <span className="mb-1 flex items-center gap-1 font-extrabold text-primary">🐱 {inBasket ? "In your basket" : "Already in your deck"}</span>
          <span className="block font-bold text-foreground">Meaning: <span className="font-medium">{vocabulary?.meaning ?? highlight?.meaning ?? "Ready for AI analysis"}</span></span>
          {!inBasket && <>
            <span className="mt-2 block text-muted-foreground">Learning progress · {highlight?.retention ?? retention}% retention</span>
            <span className="mt-1 block h-1.5 overflow-hidden rounded-full bg-muted"><span className="block h-full rounded-full bg-primary" style={{ width: `${highlight?.retention ?? retention}%` }} /></span>
            <span className="mt-1 block text-muted-foreground">Last reviewed: {highlight?.lastReviewed ?? (mastery < 2 ? "Needs a first review" : "Recently")}</span>
          </>}
          <button type="button" onClick={() => onReview(token)} className="pointer-events-auto mt-2 w-full rounded-lg bg-primary px-2 py-1.5 font-bold text-primary-foreground hover:brightness-95">🐱 Review with Pip</button>
        </span>
      </span>
    );
  })}</>;
...
export function ReaderPage() {
  ...
  const savedVocabularyByWord = useMemo(
    () => new Map(state.words.map((word) => [word.word.toLowerCase(), word])),
    [state.words],
  );
  const basketWords = useMemo(() => new Set(basket.map((item) => item.word.toLowerCase())), [basket]);
  ...
  async function saveAll() {
    ...
    const nextHighlights = Object.fromEntries(preview.map((card) => [card.word.toLowerCase(), {
      meaning: card.meaning,
